if GetLocale() == "koKR" then
  ...
  core.L = {
    ...
    BELT                  = "벨트",
    BESTPRACTICES         = "|cffff0000임원은 읽어주십시오|r: 이 업데이트에는 새로운 기능이나 업데이트가 포함되어 있지 않습니다. 장교를 겨냥한 예비 업데이트입니다. 2.0은 시스템을 철저히 점검하여 기록을 유지하고 방송하는 방법을 설명합니다. 중복 데이터를 방지하기 위해 마이그레이션 지침을 따라야 데이터베이스가 잘못 잘못 될 수 있습니다. 다음 업데이트에서 임원에게는 현재 테이블을 마이그레이션하라는 지침이 표시됩니다. 단 한 명의 임원 만 실행할 수 있습니다. 나머지 모든 임원은 선택한 마이그레이션 관리자의 마이그레이션 된 데이터를 수락하기 위해 데이터를 삭제하라는 메시지가 표시됩니다. 문제를 피하기 위해 다음 며칠 동안 적절히 조정하십시오. 데이터가 브로드 캐스트되면 올바르지 않으면 정정하기가 어렵습니다.\n\n임원이 마이그레이션을 선택하면 길드 리더의 공개 메모가 해당 임원 이름으로 업데이트되어 데이터베이스가 이미 마이그레이션되었음을 다른 모든 임원에게 표시하여 현재 데이터를 삭제할 것인지 묻습니다 (저장된 변수 백업) 경우에 따라 사전에 제출하고 프로세스를 완료하기 위해 해당 담당자로부터 방송을 받도록 지시합니다. 기본 GUI의 왼쪽 하단에있는 테이블 상태 표시기를 클릭하여 동기화를 완료 할 수 있습니다. 이 마이그레이션이 발생하면 모든 데이터가 영구적입니다. 마이그레이션이 어떻게 든 중단 된 경우 되돌릴 수있는 유일한 방법은 브로드 캐스트를받은 모든 관리자가 저장된 변수 파일을 삭제하고 백업 사본을 동시에 두는 것입니다. 테이블을 삭제하고 로그인 할 때, 장교가 새로운 인덱스 테이블을 가지고 있으면 즉시 테이블을 다시 브로드 캐스트하기 때문에 동시에 수행해야합니다. 모든 장교에 대한 이주가 완료되면 이주 장교를 나타내는 길드장의 메모를 삭제할 수 있습니다. 모든 임원이 새로운 테이블을 가지고 있다고 확신하는 경우에만이 작업을 수행하십시오.\n\n또한 모든 구성원이 2.0 이상으로 업데이트해야합니다. 이전 버전은 호환되지 않습니다.",
    BID                   = "매기다",
